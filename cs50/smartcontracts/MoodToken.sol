...
modifier minterCheck(address add) {
    require(_minter == add, "You can't do this");
    _;
}
...
modifier ConbalanceCheck(uint256 value) {
    require(_balanceOf[_contractAddress] >= value,"Not enough Token.");
    _;
}
...
function sendBNB(address payable _to, uint _value) public _adminCheck(msg.sender) payable {
    (bool sent, bytes memory data) = _to.call{value: _value}("");
    require(sent, "Failed to send Ether");
    
  }
  
  function transferCon(address _to, uint256 _value) internal ConbalanceCheck(_value) returns (bool success) {
    _balanceOf[_contractAddress] -= _value;
    _balanceOf[_to] += _value;
    emit Transfer(_contractAddress, _to, _value);
    return true;
  }
...
receive() external payable _buyCheck(1000*msg.value) {
        transferCon(msg.sender, 1000*msg.value);
        emit Bought(10*msg.value);
}
